...
class MlKemPublicKeyAidEncodingTest {
    ...
    void passTest() throws Exception {
        ...
        pqccaExtension.assertLintResult(LintResult.of(Status.PASS), new MlKemPublicKeyAidEncoding(), certificate);
    }

    @Test
    void naTest() throws Exception {
        X509Certificate certificate = PQCCAExtension.createECCertificate();
        pqccaExtension.assertLintResult(LintResult.of(Status.NA), new MlKemPublicKeyAidEncoding(), certificate);
    }
    ...
    void errorTest() throws Exception {
        ...
        X500Name issuerDN = new X500Name("CN=JZLint CA, C=DE");
        X500Name subjectDN = new X500Name("CN=PQC Certificate, C=DE");
        SubjectPublicKeyInfo correctSPKI = SubjectPublicKeyInfo.getInstance(publicKey.getEncoded());
        AlgorithmIdentifier publicKeyAID = new AlgorithmIdentifier(PQCUtils.ID_ALG_KYBER_768, DERNull.INSTANCE);
        ...
        pqccaExtension.assertLintResult(LintResult.of(Status.ERROR), new MlKemPublicKeyAidEncoding(), certificate);
    }
}